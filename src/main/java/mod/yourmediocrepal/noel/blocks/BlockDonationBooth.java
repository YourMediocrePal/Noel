    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!worldIn.isRemote) {
            ...
            if (!player.isCreative()) {
                ItemEntity donation_booth = new ItemEntity(worldIn, (double) pos.getX() + 0.5D, (double) pos.getY() + 1D, (double) pos.getZ() + 0.5D, new ItemStack(state.getBlock(), 1));
                worldIn.addEntity(donation_booth);
            }
            worldIn.playSound((PlayerEntity)null, pos.getX(), pos.getY(), pos.getZ(), SoundEvents.BLOCK_METAL_BREAK, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
    }
}
